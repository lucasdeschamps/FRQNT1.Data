...
Deschamps_2021_Soil <- Deschamps_2021_Soil %>%
  ...
  mutate(theta_sat = ifelse(Horizon == "0-5", 0.9126266, 1)) %>%
...
Deschamps_2021_Soil_sat_draws <- Deschamps_2021_Soil %>%
  ## Select relevant variables
  select(-.chain, - .iteration) %>%
  mutate(Horizon = factor(Horizon,
                          levels = c("0-5", "5-10", "10-15", "15-300")))

usethis::use_data(Deschamps_2021_Soil_sat_draws, overwrite = TRUE)
